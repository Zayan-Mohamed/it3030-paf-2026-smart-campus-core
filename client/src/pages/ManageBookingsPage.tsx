...
export const ManageBookingsPage = () => {
  ...
  const getAvailableLocations = () => {
    ...
    bookings.forEach((booking) => {
      ...
      if (statusMatch && dateMatch) {
        availableLocations.add(booking.facilityLocation);
      }
    });
    return Array.from(availableLocations).sort((a, b) => a.localeCompare(b));
  };
  ...
  const getAvailableDates = () => {
    ...
    bookings.forEach((booking) => {
      const statusMatch = !statusFilter || booking.status === statusFilter;
      const locationMatch = !locationFilter || booking.facilityLocation === locationFilter;
  ...
  const filteredBookings = bookings.filter((booking) => {
    const statusMatched = !statusFilter || booking.status === statusFilter;
    ...
    return statusMatched && dateMatched && locationMatched;
  }).sort((first, second) => {
  ...
  const getBookingsForDate = (date: Date) => {
    ...
    return filteredBookings.filter((booking) => toLocalDateKey(booking.startTime) === dateStr);
  };
  ...
  const renderCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          key={day}
          className={`border-2 p-2 rounded-lg text-xs font-medium flex flex-col min-h-24 ${
            holiday
        ...
        >
          ...
          {holiday && (
            <div className="text-black text-xs font-bold whitespace-normal overflow-hidden mb-1 leading-tight">
              {holiday.name}
            </div>
          )}
          <div className="space-y-2 flex-1">
            {dayBookings.map((booking) => (
              <div
                ...
                className={`px-2 py-1 rounded-md text-black text-xs font-bold ${bookingStatusBackgroundColor(booking.status)} shadow-md leading-snug border border-black/20 cursor-pointer hover:shadow-lg hover:scale-105 transition-all`}
                title={`${booking.userName} | ${booking.facilityLocation} | ${formatTime(booking.startTime)} - ${formatTime(booking.endTime)} | ${booking.status}`}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
              ...
              >
                <div className="truncate font-bold text-xs">{booking.status}</div>
                <div className="text-xs text-black font-semibold leading-tight">
                  {formatTime(booking.startTime)} - {formatTime(booking.endTime)}
                </div>
                <div className="text-xs text-black font-semibold truncate leading-tight">
    ...
    <div className="dashboard-layout">
      ...
      <main className="dashboard-main">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          {selectedBookingId && (
            <>
              ...
              {bookings.find((b) => b.id === selectedBookingId) ? (
                ...
                <Card>
                  <CardContent className="py-12 text-center text-sm text-slate-500">Booking not found</CardContent>
                </Card>
              )}
            </>
          )}
          ...
          {/* Calendar View */}
          {showCalendar && !selectedBookingId && (
            <Card className="mb-6 border-slate-200/80 shadow-lg shadow-slate-900/5">
              <CardHeader>
          ...
          {/* Bookings List - Only show when calendar view is hidden and no booking is selected */}
          {!showCalendar && !selectedBookingId && (
            <>
              {loading ? (
            <Card>
